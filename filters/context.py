...
class MessageContext:
    ...
    def __init__(self, client, event, chat_id, rule, metadata=None):
        """
        初始化消息上下文
        ...
        # 评论区元数据(用于评论区转发功能)
        # 如果传入了 metadata，使用传入的值，否则使用默认值
        if metadata and 'comment_metadata' in metadata:
            self.comment_metadata = metadata['comment_metadata']
        else:
            self.comment_metadata = {
                'is_comment': False,           # 是否是评论区消息
                'original_channel_chat_id': None,  # 原频道的 telegram_chat_id
                'original_message_id': None    # 原频道消息的 message_id
            }
